...
2. 슬라이싱(slicing) : 시퀀스 자료형에서 일부를 잘라내어 
                      동일한 자료형으로 반환하는 기능.
[start : stop : step]  => start에서 시작해서 stop 이전까지 step 간격으로 추출
'''
...
# city(리스트) + num1(튜플)
# result = city + num1    # 오류발생, 서로 다른 자료형은 합칠 수 없다.
...
"""
5. 멤버 유무 검사 : 시쿠너스 자료형에 특정 자료가 있는지 알려주는 기능
자료 in 자료형
"""
